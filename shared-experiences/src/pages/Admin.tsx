...
type HomeFeaturedPayload = {
  heroImage: string;
  heroSubheaderHeading: string;
  heroSubheaderSubheading: string;
  heroSubheaderEyebrow: string;
  featureHeaderEyebrow: string;
  featureHeaderHeading: string;
  featureHeaderSubheading: string;
  cards: Array<{
    image: string;
...
type HomeFeaturedApiPayload = {
  ...
  cards: HomeFeaturedPayload["cards"];
};

const HOME_FEATURED_DEFAULT_CARDS: HomeFeaturedPayload["cards"] = [
  { image: "", fallbackImage: "", title: "", category: "", excerpt: "", date: "", link: "", readMoreLabel: "" },
  { image: "", fallbackImage: "", title: "", category: "", excerpt: "", date: "", link: "", readMoreLabel: "" },
  { image: "", fallbackImage: "", title: "", category: "", excerpt: "", date: "", link: "", readMoreLabel: "" },
];

const toHomeFeaturedUiPayload = (data: HomeFeaturedApiPayload): HomeFeaturedPayload => ({
  heroImage: data.heroImage || "",
  heroSubheaderHeading: data.heroTitle || "",
  heroSubheaderSubheading: data.heroSubtitle || "",
  heroSubheaderEyebrow: data.heroCategory || "",
  featureHeaderEyebrow: data.bubbleHeading || "",
  featureHeaderHeading: data.bubbleTitle || "",
  featureHeaderSubheading: data.bubbleDescription || "",
  cards: Array.isArray(data.cards) && data.cards.length === 3
    ? data.cards.map((card) => ({ ...card, fallbackImage: card.fallbackImage ?? "" }))
    : HOME_FEATURED_DEFAULT_CARDS,
});

const toHomeFeaturedApiPayload = (data: HomeFeaturedPayload): HomeFeaturedApiPayload => ({
  heroImage: data.heroImage || "",
  heroTitle: data.heroSubheaderHeading || "",
  heroSubtitle: data.heroSubheaderSubheading || "",
  heroCategory: data.heroSubheaderEyebrow || "",
  bubbleHeading: data.featureHeaderEyebrow || "",
  bubbleTitle: data.featureHeaderHeading || "",
  bubbleDescription: data.featureHeaderSubheading || "",
  cards: data.cards,
});
...
export function AdminContent({ onSessionExpired, onLogout }: { onSessionExpired: () => void; onLogout: () => void }) {
  ...
  const [homeFeatured, setHomeFeatured] = React.useState<HomeFeaturedPayload>({
    heroImage: "",
    heroSubheaderHeading: "",
    heroSubheaderSubheading: "",
    heroSubheaderEyebrow: "",
    featureHeaderEyebrow: "",
    featureHeaderHeading: "",
    featureHeaderSubheading: "",
    cards: HOME_FEATURED_DEFAULT_CARDS.map((card) => ({ ...card })),
  });
  ...
  React.useEffect(() => {
    async function load() {
      ...
      try {
        ...
        if (homeRes.ok) {
          const homeData = (await homeRes.json()) as HomeFeaturedApiPayload | null;
          if (homeData) {
            setHomeFeatured(toHomeFeaturedUiPayload(homeData));
          }
        }
  ...
  const handleSaveHomeFeatured = async () => {
    ...
    try {
      const res = await fetch("/api/posts?home=1", {
        ...
        credentials: "include",
        body: JSON.stringify(toHomeFeaturedApiPayload(homeFeatured)),
      });
      if (!res.ok) {
      ...
      if (updated) {
        setHomeFeatured(toHomeFeaturedUiPayload(updated as HomeFeaturedApiPayload));
      }
      await saveSiteSettings(undefined, { silent: true });
      ...
      {timeoutWarning && (
        <div className="mb-6 max-w-5xl mx-auto p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          <p className="text-sm text-yellow-800">
            Warning: Your session will expire in 3 minutes due to inactivity. Move your mouse or click to extend your session.
          </p>
        </div>
      )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-6">
              ...
              <div className="space-y-3 bg-card border border-border rounded-lg p-4">
                ...
                <Input
                  placeholder="Eyebrow"
                  value={homeFeatured.heroSubheaderEyebrow}
                  onChange={(e) => setHomeFeatured({ ...homeFeatured, heroSubheaderEyebrow: e.target.value })}
                />
                <Input
                  placeholder="Heading"
                  value={homeFeatured.heroSubheaderHeading}
                  onChange={(e) => setHomeFeatured({ ...homeFeatured, heroSubheaderHeading: e.target.value })}
                />
                <Textarea
                  placeholder="Subheading"
                  value={homeFeatured.heroSubheaderSubheading}
                  onChange={(e) => setHomeFeatured({ ...homeFeatured, heroSubheaderSubheading: e.target.value })}
                  rows={2}
                />
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-3 bg-card border border-border rounded-lg p-4">
                ...
                <Input
                  placeholder="Eyebrow"
                  value={homeFeatured.featureHeaderEyebrow}
                  onChange={(e) => setHomeFeatured({ ...homeFeatured, featureHeaderEyebrow: e.target.value })}
                />
                <Input
                  placeholder="Heading"
                  value={homeFeatured.featureHeaderHeading}
                  onChange={(e) => setHomeFeatured({ ...homeFeatured, featureHeaderHeading: e.target.value })}
                />
                <Textarea
                  placeholder="Subheading"
                  value={homeFeatured.featureHeaderSubheading}
                  onChange={(e) =>
                    setHomeFeatured({ ...homeFeatured, featureHeaderSubheading: e.target.value })
                  }
                  rows={2}
                />
                <p className="text-xs text-muted-foreground">
                  Saved with the Home Featured section above.
                </p>
              </div>
